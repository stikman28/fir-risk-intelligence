...
Red Canary recorded an 850% increase in identity threat detections, making identity their #1 threat category. IBM independently documents credential theft as "at the center of many prominent campaigns," with over 300,000 ChatGPT credentials discovered on dark web markets. The attack doesn't start with malware. It starts with a valid login.

Then the contrarian finding the security community needs to hear: AI currently benefits defenders more than attackers. Red Canary's data shows AI-enhanced attacks don't fundamentally change how threats present themselves — existing detection methods remain effective. IBM confirms AI has not changed the fundamentals of cyberattack campaigns — the transformation is democratization of capability, not new methods. But IBM also warns this advantage is temporary. AI is dramatically increasing the speed, scale, and efficiency of adversary operations. Organizations that deploy AI to defend fundamentals now will have a structural advantage. Those waiting will find themselves behind.
...
IBM X-Force analyzes the global attack surface through incident response and dark web intelligence. Red Canary analyzes over 110,000 confirmed threats across 4.5 million protected assets. One sees the strategy. The other sees the ground truth.

Together, they reveal that the security industry is building castles on quicksand.

Start with the open doors. Exploitation of public-facing applications surged 44% to become the top initial access vector. Here's the number that should keep security leaders awake: 56% of disclosed vulnerabilities could be exploited without any authentication. Not sophisticated zero-days. Not advanced exploit chains. Misconfigured APIs, exposed management interfaces, default credentials on internet-facing applications. The kind of basic configuration failures that appear in every security audit but never make it to the top of the remediation queue. Attackers aren't breaking in. They're walking through doors nobody locked.
...
Then the finding the security community needs to hear: AI currently benefits defenders more than attackers. Red Canary's analysis finds that AI-enhanced attacks don't fundamentally change how threats present themselves. Existing detection methods remain effective because adversaries are using AI to accelerate existing techniques, not invent new ones. IBM confirms AI has not changed the fundamentals of cyberattack campaigns — the real shift is democratization of advanced capability.

But the advantage is temporary. IBM describes AI as a force multiplier that has dramatically increased the speed, scale, and efficiency of adversary operations. The organizations that use this window to deploy AI defensively — automating identity monitoring, configuration compliance, response to commodity patterns — will have a structural advantage when that window closes.
...
- CONFIRMED: 44% increase in public-facing app exploitation (#1 vector at 40% of cases), 109 ransomware groups, 49% YoY increase, 27.7% manufacturing, 300K+ ChatGPT credentials, 850% identity attack growth, 110K threats
- CORRECTED: "56% no-auth" applies to disclosed vulnerabilities (not just exploited ones) — wording updated
- CORRECTED: "4.5M endpoints" is actually "4.5M endpoints, identities, and cloud assets" — simplified to "protected assets"
- CORRECTED: IBM quotes were paraphrased by agent — replaced with verified quotes from report
- Red Canary CONFIRMED: "AI benefits defenders more than adversaries" is a direct quote (p.3). "AI-powered threats represent an evolution in tooling, not a revolution in attack techniques" (p.9). Identity at 53% of detection volume.
