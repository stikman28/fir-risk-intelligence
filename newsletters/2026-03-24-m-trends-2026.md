# FIR Risk Tuesday E85

**Publish Date:** March 24, 2026
**Source:** [Mandiant M-Trends 2026](https://cloud.google.com/security/resources/m-trends) (March 2026)
**Analysis:** FIR Risk Platform

---

# FIR Risk E85 — The Responder's Report

![FIR Risk E85 - The Responder's Report](images/e85-m-trends-2026.png)
...
This is our fifth and final deep dive into a 2026 annual threat report, completing the series that began with [Unit 42 (E81)](/tuesday/e81-unit42-incident-response-debrief/), [Cloudflare (E82)](/tuesday/e82-cloudflare-threat-landscape/), [CrowdStrike (E83)](/tuesday/e83-crowdstrike-global-threat-report/), and [Picus (E84)](/tuesday/e84-digital-parasite/). Five vendors, five perspectives, one conclusion: the threat landscape has fundamentally shifted — and the organizations still defending against yesterday's threats are already compromised.
...
This is our fifth and final deep dive into a 2026 annual threat report. The convergence across all five is now definitive:
...
1. **Prepare for voice-based attacks.** Email phishing dropped to 6% while vishing rose to 11%. Update help desk procedures with mandatory out-of-band identity verification for all credential and MFA changes. Train staff to recognize voice-based social engineering — not just suspicious links. Simulate vishing attacks, not just phishing emails.

2. **Audit your recovery denial exposure today.** Can attackers reach your backup management servers from your production network? Are backup service account credentials stored in the same systems attackers would compromise? Test whether you can recover without your primary identity provider, your virtualization management plane, or your backup platform interface. If the answer is no to any of these, you have an existential gap.

3. **Treat low-impact alerts as potential ransomware precursors.** The 22-second handoff means a FAKEUPDATES infection or a suspicious browser download can escalate to full ransomware before the alert is triaged. Build correlation rules that link low-impact initial access TTPs to known hand-off patterns. A minor infection today can be a ransomware attack in 22 seconds.

4. **Pursue Active Resilience, not Fragile Prevention.** Mandiant's Resilience Maturity Matrix identifies four organizational states. Most organizations are in "Fragile Prevention" — strong perimeters but no isolated recovery capability. A single control plane bypass results in an unrecoverable event. The target state is "Active Resilience": hardened identity, recovery architecturally severed from the attack surface, balanced friction and recovery.

5. **Harden the virtualization layer as Tier-0.** Attackers are targeting hypervisors for data theft, credential extraction via "Snapshot Mounting," and mass encryption of VM datastore files. Sever AD integration where possible. Enforce dedicated out-of-band management with local MFA-protected accounts. Forward virtualization platform logs to your SIEM — Mandiant found these are routinely excluded from enterprise logging.

6. **Patch enterprise web applications as zero-day targets.** The top three exploited vulnerabilities in 2025 were zero-days in SAP NetWeaver, Oracle EBS, and Microsoft SharePoint. These are the crown jewels — centralized access to financial data, business operations, and internal documents. Prioritize patching and monitoring of internet-facing enterprise platforms over endpoint-focused controls.

7. **Plan for two adversary speeds simultaneously.** The 22-second ransomware handoff and the 390-day espionage dwell time require different detection strategies. Automated, real-time correlation for fast-moving financially motivated operations. Long-term behavioral baselining and proactive threat hunting for slow-moving espionage. One detection approach cannot cover both.
...
```
When Mandiant arrives at the scene of a breach, they're finding something new: the attack started with a phone call.

M-Trends 2026 is built on over 500,000 hours of frontline incident response — 83 campaigns across 73 countries. This is the fifth and final annual threat report we've analyzed this year, completing a series that includes Unit 42, Cloudflare, CrowdStrike, and Picus. Mandiant brings something the others can't: the view from inside the compromised environment.

Three findings define this year's report.

Voice phishing rose to 11% of initial infection vectors. Email phishing collapsed to 6% — down from 22% in 2022. Your multi-million dollar email security stack is increasingly irrelevant when the adversary picks up the phone. UNC3944 (Scattered Spider) specifically researched help desk staff by name, then called impersonating employees needing password resets. The function designed to restore access became the function that granted it to attackers.

Ransomware operators have industrialized recovery denial. They're not just encrypting your data — they're destroying your ability to recover. Mandiant documented attackers compromising backup management servers, wiping millions of backup objects from cloud storage, and seizing virtualization management planes. In one case, backup data existed on immutable storage but was inaccessible because attackers deleted the backup configurations. Organizations are forced to disconnect their own identity providers to halt the spread — triggering self-inflicted business outages. You're disrupted whether you pay or not.

E84 showed ransomware encryption dropped 38%. M-Trends shows why: they don't need to encrypt when they can erase your recovery path.

Malware now queries AI during execution. PROMPTFLUX and PROMPTSTEAL actively query large language models mid-execution to modify behavior and evade detection in real time. This isn't theoretical AI risk. These are observed malware families from 2025 incident response engagements.

The most striking number: the median handoff between an initial access broker and a ransomware operator is now 22 seconds. Down from 8+ hours in 2022. A 1,300x compression. A minor browser infection can become a full ransomware deployment before the first alert is triaged.

Meanwhile, dwell time rose for the first time since 2010 — to 14 days globally. PRC-linked espionage operations using the BRICKSTORM backdoor averaged 390 days undetected. The adversary has split into two speeds: 22 seconds for ransomware, 390 days for espionage. Both are succeeding.

Five reports. Five vendors. One conclusion: the threat landscape has permanently shifted. Voice over email. Recovery denial over encryption. AI-adaptive malware over static payloads. The organizations still defending against yesterday's threats are already compromised.

Full analysis — FIR Risk Tuesday E85: The Responder's Report.

#cybersecurity #threatintelligence #incidentresponse #ransomware #identitysecurity #riskmanagement #CISO #infosec #cloudsecurity
```
...
Mandiant's M-Trends 2026 is built on over 500,000 hours of incident response. 83 campaigns. 73 countries. This is the report from the people who actually walk into compromised environments and figure out what happened.

What they found in 2025 should change how every organization thinks about risk.

Start with the phone. Voice phishing is now the #2 initial infection vector at 11%. Email phishing collapsed to 6% — down from 22% three years ago. Scattered Spider researched help desk staff by name, called impersonating employees, and talked their way past MFA. The help desk — designed to restore access — became the front door for adversaries.

In cloud-specific investigations, voice phishing was #1 at 23%. The cloud that organizations migrated to for better security is being breached primarily through phone calls.

Then the ransomware evolution. E84 showed encryption dropped 38%. M-Trends shows why: they don't need to encrypt when they can destroy your ability to recover. Mandiant calls it "Trusted Service Infrastructure" targeting — backups, identity services, virtualization management planes. All three, systematically, before encryption begins.

In one case, attackers compromised backup management servers, extracted stored credentials, then executed broad-scale deletion commands wiping millions of backup objects. In another, they deleted backup configurations — the data existed on immutable storage but was inaccessible through the backup interface. Recovery required weeks of vendor engagement. Organizations are forced to disconnect their own identity providers to halt the spread. You're disrupted whether you pay or not.

The most striking number in the report: 22 seconds. That's the median time between an initial access broker gaining entry and a ransomware operator starting operations. Down from 8+ hours in 2022. A 1,300x compression. A minor browser infection becomes a full ransomware deployment before the first alert is triaged. Mandiant documented UNC2165 (Evil Corp-linked) going from FAKEUPDATES infection to credential dumping, lateral movement, backup destruction, and RansomHub deployment — all in a single accelerated operation.

Then the AI malware. PROMPTFLUX and PROMPTSTEAL actively query large language models during execution to modify their behavior in real time. Not AI used for recon or social engineering prep — AI integrated into the malware's runtime decision-making. QUIETVAULT targets AI development environments through NPM supply chain compromise, harvesting credentials for the tools that power enterprise AI. Mandiant is measured about this: they don't consider 2025 the year breaches were caused by AI. But the first generation of AI-powered attack tools is now confirmed in the wild.

Global median dwell time rose to 14 days from 11 — the first increase since Mandiant began tracking in 2010. PRC-linked espionage operations using BRICKSTORM averaged 390 days undetected. Salt Typhoon systematically exploited VPN and router vulnerabilities to target telecom backbones, harvesting authentication protocols with packet capture tools native to the compromised appliances. The adversary has split into two speeds: 22 seconds for ransomware, 390 days for espionage.

This is our fifth and final 2026 threat report deep dive. Unit 42 showed the speed. Cloudflare showed the infrastructure. CrowdStrike showed the scale. Picus showed the technique. Mandiant shows what the responders actually find when they arrive at the scene.

Five reports. Five vendors. One conclusion: voice over email, recovery denial over encryption, AI-adaptive malware over static payloads. The permanent shift is complete.

Full E85 — The Responder's Report — linked below.
...
**Platform Queries:**
1. "What are the most significant findings from the Mandiant M-Trends 2026 report? What does the data reveal about how organizations are actually getting breached in 2025, and how has that changed from prior years?"
2. "M-Trends 2026 identifies voice phishing (vishing) as the second most common initial infection vector at 11%. Go deep on this — what campaigns, what techniques, what made social engineering more effective than traditional exploits for nearly 1 in 9 breaches?"
3. "Explain the shift toward recovery denial and backup targeting in ransomware operations. How are adversaries ensuring victims cannot recover without paying? What specific techniques and campaigns from M-Trends illustrate this evolution?"
4. "M-Trends 2026 documents AI integration accelerating attack lifecycles. What specific AI-driven techniques did Mandiant observe? Then step back — we've now covered five 2026 threat reports: Unit 42 (E81), Cloudflare (E82), CrowdStrike (E83), Picus (E84), and now Mandiant M-Trends (E85). What picture emerges across all five? Where does M-Trends add something the others didn't?"
5. "Based on M-Trends 2026 findings — especially the dominance of exploits, the rise of voice phishing, recovery denial targeting backups, and AI-accelerated attacks — what defensive approaches are failing and what needs to change? What would you recommend to an enterprise risk leader reading this?"

**KB Sources:**
- Mandiant M-Trends 2026 (March 2026)
- FIR Risk Tuesday E81-E84

**Fact-Check Method:**
- Full PDF review (pages 1-72) against all agent claims
- Two critical corrections applied (dwell time: 14 days not 11; industry targeting: High Tech #1 not Financial)
- 11 additional findings incorporated from direct PDF review
